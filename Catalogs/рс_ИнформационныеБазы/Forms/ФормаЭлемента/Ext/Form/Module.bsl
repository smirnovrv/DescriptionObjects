...
Функция СоздатьОбъектНаСервере(Структура)
	...
	поискОбъект = ОбщийМодульСервер.НайтиОбъект(Структура.Вид, Структура.Индентификатор); 
	
	Если ЗначениеЗаполнено(поискОбъект) Тогда 
		Возврат поискОбъект;
	Иначе	
		//Соединение = ОбщийМодульСервер.ПодключитьсяКТекущейбазе(Объект.Ссылка); временно		
